@props([
    'emitToPath' => 'components.additional-service-modal',
    'emitToMethod' => 'show',
])

<section>
    <div class="w-full text-center p-1">
        <h1 class="text-4xl font-bold tracking-tight text-gray-900 pt-6">Date Servicii</h1>
    </div>
    <div class="relative">
        <div
            ...
            <div>
                @include('common.generic-btn', [
                    ...
                    'wire_method' => handleEmitTo(
                        $emitToPath,
                        $emitToMethod,
                        handleModalAddData('add', $additionalServices->items())),
                    'class' =>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                ...
                @foreach ($additionalServices->items() ?? [] as $index => $rowData)
                    <tr
                        class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        @foreach (handleTableBody($rowData->getAttributes(), ['id', 'nume']) ?? [] as $key => $value)
                            <td class="px-6 py-4">

                                {{ $key === 'id' ? handleLivewireTableIds($index, $additionalServices) : $value }}
                        ...
                        <td class="px-6 py-4 flex justify-center">
                            ...
                            @include('common.generic-btn', [
                                ...
                                'wire_method' => handleEmitTo(
                                    $emitToPath,
                                    $emitToMethod,
                                    handleModalData(handleLivewireTableIds($index - 1, $additionalServices),
                                        'edit',
        ...
        @if (empty($additionalServices->items()))
